...
export class CourseService {
  ...
  updateCourseDesarrollo(course: Course): Observable<any> {
    const name = course.name
    const body = { name, desarrollo: true };
    const url = `${this.coursesUrl}/${course.id}`
    return this.http.put<Course>(url, body,this.httpOptions)
    .pipe(
      tap((addedCourse: Course) => this.log(`Se actualizo = ${course.name}. a desarrollo`)),
      catchError(this.handleError<Course>("updateCourseDesarrollo"))
    );
  }

  getCoursesInProgress(): Observable<Course[]> {
    const courses =  this.http.get<Course[]>(this.coursesUrl);
    //filtrar para encontrar los objetos que contengan el objeto "desarrollo": true
